...
-   **Universal Query Builder:** Decouples GraphQL input from Hibernate Search internals, handling type-safe path resolution and predicate building.
...
-   **UI Enhancements:**
    ...
    -   Added `salary` and `age` to possible stats for the People domain.
-   **DDD-Style Mutations:**
    ...
    -   Added comprehensive unit tests (`PersonMergeServiceTest`), GraphQL mutation tests (`PersonMutationTest`), and fixed existing `PersonGraphqlTest`.
