package com.example.graphql.person.infrastructure.search;
...
@Repository
public class HibernatePersonSearchAdapter implements PersonSearchRepository {

    private static final Logger log = LoggerFactory.getLogger(HibernatePersonSearchAdapter.class);
    ...
    public HibernatePersonSearchAdapter(EntityManager entityManager, 
    ...
    public PersonSearchInternalResponse search(String text, List<SearchConditionInput> filter, List<String> facetKeys, List<String> statsKeys, int page, int size) {
        ...
        var query = searchSession.search(Person.class)
            .where(f -> f.bool(b -> {
                ...
                // Security: Inject DAC predicates directly into the query
                b.filter(dacService.getSecurityPredicate(f, rootEntity));
            }));
    ...
    private void applyFullTextSearch(SearchPredicateFactory f, BooleanPredicateClausesStep<?> bool, String text, EntityCfg rootEntity) {
        ...
        // Registry-Driven Full-Text Search
